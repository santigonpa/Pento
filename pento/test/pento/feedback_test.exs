defmodule Pento.FeedbackTest do
  use Pento.DataCase
  ...
  describe "faqs" do
    ...
    test "create_faq/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Feedback.create_faq(@invalid_attrs)
    end
    ...
    test "update_faq/2 with valid data updates the faq" do
      ...
      assert {:ok, %Faq{} = faq} = Feedback.update_faq(faq, update_attrs)
    ...
    test "update_faq/2 with invalid data returns error changeset" do
      faq = faq_fixture()
      assert {:error, %Ecto.Changeset{}} = Feedback.update_faq(faq, @invalid_attrs)
      assert faq == Feedback.get_faq!(faq.id)
    end

    test "delete_faq/1 deletes the faq" do
      faq = faq_fixture()
      assert {:ok, %Faq{}} = Feedback.delete_faq(faq)
      assert_raise Ecto.NoResultsError, fn -> Feedback.get_faq!(faq.id) end
    end

    test "change_faq/1 returns a faq changeset" do
      faq = faq_fixture()
      assert %Ecto.Changeset{} = Feedback.change_faq(faq)
    end
  end
end
